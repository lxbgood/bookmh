...
public class ImageUtil {
	public static synchronized void setViewImage(final Context context, final ImageView view,  final String imageUrl,final boolean small) {
		...
		if(small){//前景图片
			Glide.with(MyApplication.getInstance()).load(imageUrl).asBitmap().into(view);
		}else{
//			Glide.with(MyApplication.getInstance()).load(imageUrl).into(view);
			Glide.with(MyApplication.getInstance()).load(imageUrl).asBitmap().into(new SimpleTarget<Bitmap>(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL) {
				@Override
